...
#ifdef DEBUG
  #define debug(...) ljj_printf(__VA_ARGS__)
  #ifndef VERBOSE
    #define VERBOSE
  #endif
#else
...
#ifdef VERBOSE
  #define info(...) ljj_printf(__VA_ARGS__)
#else
  #define info(...)
#endif
...
int test_lock(){
	// when -DDEBUG set
	//  fprint(stderr)
	// will cause deadlock by competing the stderr object
	fprintf(stderr, "oops~\n");
	return 0xdead;
}

int test_fork(){
	// return system("echo subprocess");
	if (fork()){
		write(1, "parent\n", 7);
	} else {
		write(1, "child\n", 6);
...
uintptr_t __attribute__ ((noinline, naked)) _invoke0(void* block, void* fakelr, void**lr){
	asm volatile (
			"str    lr, [x2]\n"
			"mov    lr, x1\n"
			"stp 	xzr, xzr,  [sp, #-16]!\n"
...
uintptr_t invoke0(void* func){

	uintptr_t exit_pc = 0xdeadbeefdeadbeef;
	uintptr_t lr;
	uintptr_t cur;
	...
	if (thread_data->tls == NULL){
		thread_data->tls = get_tls();
	}
	uintptr_t block_address = scan(thread_data, (uint16_t *)func, ALLOCATE_BB);
	debug("Address of first basic block is: 0x%x\n", block_address);
	int exit_block = allocate_bb(thread_data);
	uintptr_t exit_address = (uintptr_t)&thread_data->code_cache->blocks[exit_block];
	thread_data->code_cache_meta[exit_block].source_addr = exit_pc;
  	thread_data->code_cache_meta[exit_block].tpc = exit_address;
	cur = exit_address;
	if (cur % 8 == 4){
		*(uint32_t*)cur = 0xAA0003E0; // nop
		cur += 4;
	}
	*(uint32_t*)cur = 0xA8C107E0; cur += 4; // ldp	x0, x1, [sp], #16 // NOT `ldp 	x1, x0, [sp, #16]!`
	*(uint32_t*)cur = 0x58000071; cur += 4; // ldr 	x17, #16
	*(uint32_t*)cur = 0xF9400231; cur += 4; // ldr 	x17, [x17]
	*(uint32_t*)cur = 0xD61F0220; cur += 4; // br  	x17
	*(uintptr_t*)cur = &lr; cur += 8;	// .data &lr

	if (!hash_add(&thread_data->entry_address, exit_pc, exit_address)) {
      		fprintf(stderr, "Failed to add hash table entry for newly created basic block\n");
      		while(1);
    	}


	uintptr_t ret = _invoke0(block_address, exit_pc, &lr);
